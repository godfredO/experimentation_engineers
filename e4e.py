...
class E4E:
    '''
    chapter: the chapter for which we are generating figures
    ...
    def __init__(self, chapter, base_dir="./"):
        ...
        mpl.rcParams['figure.dpi'] = 300        # dots per inch for sharpness
    ...
    def save_fig_named(self, name):
        ...
        for ext in ["svg", "eps", "png"]:
            try:
                fmt = f"{self.chapter:020}"
            except Exception:
                fmt = self.chapter 
            plt.save(f"{self.fig_dir}/CH{fmt}_{name}_owusu.{ext}", transparent=True)
